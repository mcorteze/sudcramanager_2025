...
export default function ItemResp2List({ id_matricula_eval }) {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:3001/api/archivos_leidos/itemresp2/${id_matricula_eval}`
        );
        const json = await res.json();
        setData(json);
      } catch (error) {
        console.error("Error al cargar itemresp2:", error);
      } finally {
        setLoading(false);
      }
    };
